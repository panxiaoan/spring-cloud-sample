...
echo ">>> docker run……"
docker run -d --restart=always -m 300M --memory-swap -1 -e JAVA_OPTS='-server -Xms256m -Xmx256m -Dspring.profiles.active=test' -p $PORT:$PORT --name $CONTAINER_NAME $IMAGES_NAME:$BUILD_ID
